...
void UAbilityTask_HomingProjectile::SpawnProjectile()
{
	...
	{
		UE_LOG(LogTemp, Log, TEXT("WeaponAttributeSet->GetProjectileInterval() is:: %f"), WeaponAttributeSet->GetProjectileInterval());
	...
	if (!HitTargets.Contains(OtherActor))
	{
		if (ICharacterBaseInterface *CharacterInterface = Cast<ICharacterBaseInterface>(OtherActor))
		{
			if (CharacterInterface->IsCharacterAlive() &&
				CharacterInterface->TagExactExistsInAbilityComponent(SentinelsScourgeGameplayTags::Enemy_Root))
			{
