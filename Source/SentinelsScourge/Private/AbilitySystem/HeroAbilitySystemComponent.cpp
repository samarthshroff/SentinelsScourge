...
#include "AbilitySystem/Abilities/SSGameplayAbilityInterface.h"
...
void UHeroAbilitySystemComponent::AcquireAbility(const FGameplayTag &AbilityTag)
{
	...
	if (bIsWeapon && WeaponManager != nullptr)
	{
		...
		else // Else create a new weapon (ability) and give it to the Owning Actor.
		{
			...
			//
			Cast<ISSGameplayAbilityInterface>(FindAbilitySpecFromHandle(AbilitySpecHandle)->GetPrimaryInstance())->Initialize(AttributeSet->GetCooldown());
...
void UHeroAbilitySystemComponent::Initialize(const TObjectPtr<UWeaponData> &WeaponData)
{
	...
	USSGameplayAbility::OnAbilityCooldownCompleteDelegate.AddUObject(this, &UHeroAbilitySystemComponent::OnAbilityCooldownComplete);
}
...
void UHeroAbilitySystemComponent::OnAbilityCooldownComplete(const FActiveGameplayEffect *ActiveGameplayEffect)
{
	...
	if (GameplayEffect->DurationPolicy == EGameplayEffectDurationType::HasDuration)
	{
		...
		{
			if (Tag.MatchesTag(SentinelsScourgeGameplayTags::Weapon_Cooldown))
			{
				bIsCooldownGE = true;
				break;
			}
		...
		if (bIsCooldownGE)
		{
			...
			for (FGameplayTag Tag : AssetTags)
			{
				...
				if (Weapon.IsSet())
				{
					...
					if (Weapon->Name.MatchesTagExact(SentinelsScourgeGameplayTags::Weapon_Hero_MagicWand))
					{
						return;
					}
